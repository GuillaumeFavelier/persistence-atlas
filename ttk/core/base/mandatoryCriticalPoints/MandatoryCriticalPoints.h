...
namespace ttk{
  ...
  public:
    ...
    public:
      ...
      ~Edge(){}
      const pair<int,int>& getVertexIdx() const {
        return vertexIdx_;
      }
    protected:
      pair<int,int> vertexIdx_;
    };
  public:
  ...
  struct criticalPointPairComparaison {
    bool operator()(const pair<pair<int,int>,double> &left, const pair<pair<int,int>,double> &right) {
  ...
  // TODO : template
  /// Comparison of the second member of a pair<int,double>
  ...
  class MandatoryCriticalPoints : public Debug {
    ...
    public:
      ...
      /// \pre To build these trees, the following must have been called : setVertexNumber(), fillVertexScalars(), setVertexPosition(), setTriangulation() and setSoSoffsets().
      int buildSubTrees();
      ...
      inline int fillVertexScalars(void *upperData, void *lowerData){
        ...
        #pragma omp parallel for num_threads(threadNumber_) if(isParallel_)
        #endif
      ...
      inline const vector<double>* getJoinTreeXLayout() {
        return &( mdtJoinTreePointXCoord_ );
      }

      inline const vector<double>* getJoinTreeYLayout() {
      ...
      inline const vector<double>* getSplitTreeXLayout() {
        return &( mdtSplitTreePointXCoord_ );
      }

      inline const vector<double>* getSplitTreeYLayout() {
        return &( mdtSplitTreePointYCoord_ );
      }


      inline const vector<int>* getMdtJoinTreePointComponentId() const {
        return &( mdtJoinTreePointComponentId_ );
      }
      inline const vector<int>* getMdtSplitTreePointComponentId() const {
        return &( mdtSplitTreePointComponentId_ );
      }
      inline const vector<PointType>* getMdtJoinTreePointType() const {
        return &( mdtJoinTreePointType_ );
      }
      inline const vector<PointType>* getMdtSplitTreePointType() const {
        return &( mdtSplitTreePointType_ );
      }
      inline const vector<double>* getMdtJoinTreePointLowInterval() const {
        return &( mdtJoinTreePointLowInterval_ );
      }
      inline const vector<double>* getMdtSplitTreePointLowInterval() const {
        return &( mdtSplitTreePointLowInterval_ );
      }
      inline const vector<double>* getMdtJoinTreePointUpInterval() const {
        return &( mdtJoinTreePointUpInterval_ );
      }
      inline const vector<double>* getMdtSplitTreePointUpInterval() const {
        return &( mdtSplitTreePointUpInterval_ );
      }
      inline const vector<int> *getMdtJoinTreeEdgeSwitchable() {
        return &( mdtJoinTreeEdgeSwitchable_ );
      }
      inline const vector<int> *getMdtSplitTreeEdgeSwitchable() {
        return &( mdtSplitTreeEdgeSwitchable_ );
      }

      inline bool areSaddlesSwitchables(
        const TreeType treeType,
        const int &firstId,
        const int &secondId) const {
        const double firstLower = (treeType == TreeType::JoinTree) ?
        ...
        const double firstUpper = (treeType == TreeType::JoinTree) ?
        ...
        const double secondLower = (treeType == TreeType::JoinTree) ?
        ...
        const double secondUpper = (treeType == TreeType::JoinTree) ?
      ...
      int outputLocationLikelihood(vector<int>& mandatoryVertex,
          vector<vector<int>>& mandatoryComponentVertices,
          vector<bool>& isSimplified,
          vector<pair<double,double>>& criticalInterval,
          double* locationLikelihood,
          int componentId=-1){
        const int numberOfComponents=mandatoryVertex.size();

        vector<double*> probability(numberOfBins_, nullptr);
        for(int i=0; i<numberOfBins_; ++i)
          probability[i]=static_cast<double*>((*inputProbability_)[i]);

        std::fill(locationLikelihood, locationLikelihood+vertexNumber_, 0);

        int startId=0;
        int endId=numberOfComponents;
        if(componentId>-1){
          startId=componentId;
          endId=startId+1;
        }
        ...
        for(int i=startId; i<endId; ++i){
          if(!isSimplified[i]){
            const double cmin=criticalInterval[i].first;
            const double cmax=criticalInterval[i].second;

            const int numberOfComponentVertices=mandatoryComponentVertices[i].size();
#ifdef TTK_ENABLE_OPENMP
#pragma omp parallel for num_threads(threadNumber_) if(isParallel_)
#endif
            for(int j=0; j<numberOfComponentVertices; ++j){
              const int vertexId=mandatoryComponentVertices[i][j];

              const double fmin=lowerVertexScalars_[vertexId];
              const double fmax=upperVertexScalars_[vertexId];

              if(cmin == cmax){
                if(fmin == fmax and cmin == fmin)
                  locationLikelihood[vertexId]=1;
                continue;
              }

              if(fmin == fmax) continue;

              const double deltaF=(fmax-fmin)/numberOfBins_;

              double totalArea=0.0;
              for(int k=0; k<numberOfBins_; ++k){
                totalArea+=probability[k][vertexId]*deltaF;
              }

              double s=0;
              for(int k=0; k<numberOfBins_; ++k){
                const double leftValue=fmin+k*deltaF;
                const double rightValue=leftValue+deltaF;

                const double prob=probability[k][vertexId];

                if(prob==-1) continue;

                const double area=prob*deltaF;

                // bin is inside the critical interval
                if(leftValue>=cmin and leftValue<=cmax and rightValue>=cmin and rightValue<=cmax){
                  // coeff is equal to one
                  s+=area;
                }
                // add left boundary bin
                else if(leftValue<cmin and rightValue>cmin and rightValue<cmax){
                  const double coeff=(rightValue-cmin)/deltaF;
                  s+=coeff*area;
                }
                // add right boundary bin
                else if(leftValue>cmin and leftValue<cmax and rightValue>cmax){
                  const double coeff=(cmax-leftValue)/deltaF;
                  s+=coeff*area;
                }
                // critical interval is inside a bin
                else if(leftValue<cmin and leftValue<cmax and rightValue>cmin and rightValue>cmax){
                  const double coeff=(cmax-cmin)/deltaF;
                  s+=coeff*area;
                }
              }

              locationLikelihood[vertexId]+=s/totalArea;
            }
          }
        }
      ...
      int outputMaximumLocationLikelihood(int componentId=-1){
        double* maximumLocationLikelihood=static_cast<double*>(outputMaximumLocationLikelihood_);

        outputLocationLikelihood(mandatoryMaximumVertex_,
            mandatoryMaximumComponentVertices_,
            isMdtMaximumSimplified_,
            mandatoryMaximumInterval_,
            maximumLocationLikelihood,
            componentId);

        return 0;
      }

      int outputMinimumLocationLikelihood(int componentId=-1){
        double* minimumLocationLikelihood=static_cast<double*>(outputMinimumLocationLikelihood_);

        outputLocationLikelihood(mandatoryMinimumVertex_,
            mandatoryMinimumComponentVertices_,
            isMdtMinimumSimplified_,
            mandatoryMinimumInterval_,
            minimumLocationLikelihood,
            componentId);
      ...
      inline int outputJoinSaddle(const int &id, const bool &reset = true){
        ...
        if(id<(int)mandatoryJoinSaddleVertex_.size()) {
          if(!isMdtJoinSaddleSimplified_[id]) {
            ...
            for(int i=0 ; i<(int)mandatoryJoinSaddleComponentVertices_[id].size() ; i++){
      ...
      int outputMaximum(const int &id, const bool &reset=true, const bool &parallel=true) {
        ...
        if(id<(int)mandatoryMaximumVertex_.size()) {
          ...
          for(int i=0 ; i<(int)mandatoryMaximumComponentVertices_[id].size() ; i++)
      ...
      int outputMinimum(const int &id, const bool &reset=true, const bool &parallel=true) {
        ...
        if(id<(int)mandatoryMinimumVertex_.size()) {
          if(!isMdtMinimumSimplified_[id]) {
            ...
            for(int i=0 ; i<(int)mandatoryMinimumComponentVertices_[id].size() ; i++)
      ...
      inline int outputSplitSaddle(const int &id, const bool &reset = true){
        ...
        if(id<(int)mandatorySplitSaddleVertex_.size()) {
          if(!isMdtSplitSaddleSimplified_[id]) {
            ...
            for(int i=0 ; i<(int)mandatorySplitSaddleComponentVertices_[id].size() ; i++){
      ...
      inline int setOutputCriticalPointProbabilityPointer(void *data){
        outputCriticalPointProbability_ = data;
        return 0;
      }
      ...
      inline int setClusterId(int clusterId){
        clusterId_ = clusterId;
        return 0;
      }

      inline int setNumberOfClusters(int numberOfClusters){
        numberOfClusters_ = numberOfClusters;
        return 0;
      }

      inline int setNumberOfObjects(int numberOfObjects){
        numberOfObjects_ = numberOfObjects;
        return 0;
      }

      inline int setNumberOfBins(int numberOfBins){
        numberOfBins_ = numberOfBins;
        return 0;
      }

      inline int setInputProbabilityPointer(vector<void*> *data){
        inputProbability_ = data;
        return 0;
      }

      inline int setOutputMaximumLocationLikelihoodPointer(void *data){
        outputMaximumLocationLikelihood_ = data;
        return 0;
      }

      inline int setOutputMinimumLocationLikelihoodPointer(void *data){
        outputMinimumLocationLikelihood_ = data;
        return 0;
      }
      ...
      inline int setupTriangulation(Triangulation *triangulation){
        ...
        if(triangulation_ and !triangulation_->hasPreprocessedVertexNeighbors()){
          triangulation_->preprocessVertexNeighbors();
        }
        return 0;
      }

      static int preprocessTriangulation(Triangulation *triangulation){
        if(triangulation and !triangulation->hasPreprocessedVertexNeighbors()){
          triangulation->preprocessVertexNeighbors();
        }
        return 0;
      }
      ...
      inline int setVertexPosition(const int &i, const double point[3]){
        if((int)vertexPositions_.size() != vertexNumber_)
          vertexPositions_.resize(vertexNumber_, vector<double>(3));
        if(i<vertexNumber_){
    ...
    protected:
      ...
      void                          *outputCriticalPointProbability_;

      void                          *outputMaximumLocationLikelihood_;

      void                          *outputMinimumLocationLikelihood_;

      vector<void*>                 *inputProbability_;
      ...
      vector<double>                upperVertexScalars_;
      /// Copy of the input lower scalar field converted in double.
      vector<double>                lowerVertexScalars_;
      ...
      vector<bool>                  isMdtMinimumSimplified_;
      /// Flags indicating if the mandatory join saddle component have been simplified.
      vector<bool>                  isMdtJoinSaddleSimplified_;
      /// Flags indicating if the mandatory split saddle component have been simplified.
      vector<bool>                  isMdtSplitSaddleSimplified_;
      /// Flags indicating if the mandatory maximum component have been simplified.
      vector<bool>                  isMdtMaximumSimplified_;

      // Graph
      vector<int> mdtMinimumParentSaddleId_;
      vector<int> mdtJoinSaddleParentSaddleId_;
      vector<int> mdtSplitSaddleParentSaddleId_;
      vector<int> mdtMaximumParentSaddleId_;

      Graph mdtJoinTree_;
      Graph mdtSplitTree_;

      vector<int> mdtJoinTreePointComponentId_;
      vector<int> mdtSplitTreePointComponentId_;
      vector<PointType> mdtJoinTreePointType_;
      vector<PointType> mdtSplitTreePointType_;
      vector<double> mdtJoinTreePointLowInterval_;
      vector<double> mdtSplitTreePointLowInterval_;
      vector<double> mdtJoinTreePointUpInterval_;
      vector<double> mdtSplitTreePointUpInterval_;

      vector<int> mdtJoinTreeEdgeSwitchable_;
      vector<int> mdtSplitTreeEdgeSwitchable_;

      vector<double> mdtJoinTreePointXCoord_;
      vector<double> mdtSplitTreePointXCoord_;

      vector<double> mdtJoinTreePointYCoord_;
      vector<double> mdtSplitTreePointYCoord_;
      ...
      vector<vector<int> >          mandatoryMaximumComponentVertices_;
      /// List of the vertices forming each of the mandatory split saddle components.
      vector<vector<int> >          mandatoryMinimumComponentVertices_;
      /// List of the vertices forming each of the mandatory join saddle components.
      vector<vector<int> >          mandatoryJoinSaddleComponentVertices_;
      /// List of the vertices forming each of the mandatory split saddle components.
      vector<vector<int> >          mandatorySplitSaddleComponentVertices_;
...
// template functions
template <class dataType> int MandatoryCriticalPoints::execute() {
  ...
  // compute critical point probability
  if(clusteringData_){
#ifndef TTK_ENABLE_KAMIKAZE
    if(!outputCriticalPointProbability_)
      return -1;
#endif

    int* clusteringDataArray=static_cast<int*>(clusteringData_);
    const int numberOfElements=std::count(clusteringDataArray,
        clusteringDataArray+numberOfObjects_,
        clusterId_);

    double* criticalPointProbability=static_cast<double*>(outputCriticalPointProbability_);
    const double prob=double(numberOfElements)/numberOfObjects_;
    for(int j=0; j<vertexNumber_; ++j)
      criticalPointProbability[j]=prob;
  }
  ...
  #ifdef TTK_ENABLE_OPENMP
  #pragma omp parallel sections if(isParallel_)
  #endif
  {
    #ifdef TTK_ENABLE_OPENMP
    #pragma omp section
  ...
  fill(mandatoryMinimumComponentVertices_.begin(),
    mandatoryMinimumComponentVertices_.end(),
    vector<int>());
  mandatoryJoinSaddleComponentVertices_.resize(mandatoryJoinSaddleVertex_.size());
  fill(mandatoryJoinSaddleComponentVertices_.begin(),
    mandatoryJoinSaddleComponentVertices_.end(),
    vector<int>());
  mandatorySplitSaddleComponentVertices_.resize(mandatorySplitSaddleVertex_.size());
  fill(mandatorySplitSaddleComponentVertices_.begin(),
    mandatorySplitSaddleComponentVertices_.end(),
    vector<int>());
  mandatoryMaximumComponentVertices_.resize(mandatoryMaximumVertex_.size());
  fill(mandatoryMaximumComponentVertices_.begin(),
    mandatoryMaximumComponentVertices_.end(),
    vector<int>());
